...
async function handleSandboxEvent(event: E2BSandboxEvent): Promise<void> {
  ...
  switch (eventLabel) {
    ...
    case 'pause':
      console.log(`[e2b-webhook] Sandbox paused: ${sandboxId}`);
      // 沙箱已暂停（计费暂停）
      break;

    case 'resume':
      console.log(`[e2b-webhook] Sandbox resumed: ${sandboxId}`);
      // 沙箱已恢复（计费恢复）
      break;
 ...
 * E2B Webhook 接收端点
 *
 * 接收 E2B 发送的沙箱生命周期事件通知
 *
 * 配置方式：
 * 1. 登录 E2B Dashboard (https://e2b.dev)
 * 2. 进入 Settings > Webhooks
 * 3. 添加 Webhook URL: https://your-domain.com/api/webhooks/e2b
 * 4. 选择事件类型：Sandbox Lifecycle Events
 * 5. 保存 Webhook Secret 到环境变量 E2B_WEBHOOK_SECRET
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (event.eventCategory !== 'lifecycle') {
      console.warn('[e2b-webhook] Unsupported event category:', event.eventCategory);
      return NextResponse.json(
