              ...
              try {
                ...
                if (data.type === 'status') {
                  ...
                  setGenerationProgress(prev => {
                    ...
                    // 🔥 改进：使用 \s+ 支持灵活空白
                    const lastFileMatch = newStreamedCode.match(/<file\s+path="([^"]+)">([^]*?)$/);
...
//   };
  ...
  const handlePlanConfirm = async (
    ...
    // 重置 Plan 状态
    setPlanMode('idle');
    // 确保展示 Code 视图，避免用户停留在 View 导致“生成中但看不到进度”
    setActiveTab('generation');
    ...
    try {
      ...
      // SSE 解析：支持跨 chunk 拼接，避免 JSON 被截断导致丢事件
      const readSse = async (response: Response, onEvent: (data: any) => Promise<void> | void) => {
      ...
      // 1) 获取 manifest：优先用 Plan 解析结果，失败则单独请求 manifest 模式兜底
      let manifest: PlanGenerationResponse['plan']['suggestedManifest'] =
      ...
      if (!manifest || manifest.length === 0) {
        ...
        await readSse(manifestResponse, async (data) => {
          if (data.type === 'status' && typeof data.message === 'string') {
            setGenerationProgress(prev => ({ ...prev, status: data.message }));
          }
          if (data.type === 'manifest_complete' && Array.isArray(data.manifest)) {
            fetchedManifest = data.manifest;
          }
          if (data.type === 'error') {
            throw new Error(data.error || '文件清单生成失败');
          }
        });
      ...
      for (let fileIndex = 0; fileIndex < manifest.length; fileIndex++) {
        ...
        setGenerationProgress(prev => ({
          ...prev,
          status: `Generating ${filePath}`,
          currentFile: { path: filePath, content: '', type: fileType }
        }));

        let fileResult: { path: string; content: string } | null = null;
        ...
        await readSse(fileResponse, async (data) => {
          if (data.type === 'status' && typeof data.message === 'string') {
            setGenerationProgress(prev => ({ ...prev, status: data.message }));
          }
          // 🔥 打字机效果：处理代码块事件
          if (data.type === 'codeChunk' && typeof data.chunk === 'string') {
            setGenerationProgress(prev => ({
              ...prev,
              streamedCode: prev.streamedCode + data.chunk
            }));
          }
          if (data.type === 'file_complete' && data.file && typeof data.file.path === 'string') {
            fileResult = { path: data.file.path, content: String(data.file.content || '') };
          }
          if (data.type === 'error') {
            throw new Error(data.error || `文件生成失败: ${filePath}`);
          }
        });
